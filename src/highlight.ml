    ...
    let kind =
      ...
      | None ->
         ...
         | None, Some b, Some e ->
            ...
            in
            Delim {
                delim_begin = Pcre.regexp ~iflags (Ezjsonm.get_string b);
                delim_end = Pcre.regexp ~iflags (Ezjsonm.get_string e);
                delim_patterns =
                  begin match find "patterns" obj with
                  | None -> []
                  | Some v ->
                     Ezjsonm.get_list
                       (fun x -> Ezjsonm.get_dict x |> patterns_of_plist) v
                  end;
                delim_name =
                  Option.map Ezjsonm.get_string (find "name" obj);
                delim_content_name =
                  Option.map Ezjsonm.get_string (find "contentName" obj);
                delim_pats = [];
